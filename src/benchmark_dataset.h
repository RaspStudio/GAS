...
#include "filter.h"      // for BaseFilterFunctor, RangeGasFilterFunctor, TagGasFilterFunctor

namespace gaslib {

// MetaSet: Container for secondary metadata attributes in composed filtering.
// Used by PostFilterIndex and GasIndexExtend to store additional attribute data.
struct MetaSet {
    std::vector<label_t> labels;
};

// Forward declarations for helpers used by MetaSet loaders.
inline auto load_bmeta(const std::string& meta_path, size_t n_elements) -> std::vector<meta_t>;

template <typename MetaT>
inline auto load_bmeta_as(const std::string& meta_path, size_t n_elements) -> std::vector<MetaT>;

// Load metadata and encode as labels (meta<<32 | id) into a MetaSet.
// This is used by composed filtering indices as a second attribute storage.
inline auto load_metaset(const std::string& meta_path, size_t n_elements) -> MetaSet {
    MetaSet ms;
    ms.labels.resize(n_elements);
    auto meta = load_bmeta(meta_path, n_elements);
    for (size_t i = 0; i < n_elements; ++i) {
        uint32_t id = static_cast<uint32_t>(i);
        uint32_t m = static_cast<uint32_t>(meta[i]);
        ms.labels[i] = (static_cast<label_t>(m) << 32) | id;
    }
    return ms;
}

template <typename MetaT>
inline auto load_metaset_as(const std::string& meta_path, size_t n_elements) -> MetaSet {
    MetaSet ms;
    ms.labels.resize(n_elements);
    auto meta = load_bmeta_as<MetaT>(meta_path, n_elements);
    for (size_t i = 0; i < n_elements; ++i) {
        uint32_t id = static_cast<uint32_t>(i);
        uint32_t m = static_cast<uint32_t>(meta[i]);
        ms.labels[i] = (static_cast<label_t>(m) << 32) | id;
...
// Templated metadata loader (for composed/extended meta types)
template <typename MetaT>
inline auto load_bmeta_as(const std::string& meta_path, size_t n_elements) -> std::vector<MetaT> {
    static_assert(std::is_trivially_copyable_v<MetaT>, "MetaT must be trivially copyable");
    std::ifstream in(meta_path, std::ios::binary);
    if (!in.is_open()) {
        std::cerr << "Error: Failed to open metadata file " << meta_path << std::endl;
        std::exit(EXIT_FAILURE);
    ...
    std::vector<MetaT> meta_data(n_elements);
    for (size_t i = 0; i < n_elements; ++i) {
        MetaT m{};
        in.read(reinterpret_cast<char*>(&m), sizeof(MetaT));
        if (!in) {
            std::cerr << "Error: Metadata file ended unexpectedly at index " << i << std::endl;
            std::exit(EXIT_FAILURE);
        }
...
// MetaDataset: only loads per-vector metadata, without loading vectors.
// This is useful for composed filtering where a second attribute is stored separately.
template <typename MetaT = meta_t>
class MetaDataset {
public:
    MetaDataset(size_t n_elements, const std::string& meta_path)
        : n_elements_(n_elements), meta_data_(load_bmeta_as<MetaT>(meta_path, n_elements)) {}

    size_t size() const { return n_elements_; }

    MetaT get_meta(size_t index) const {
        assert(index < n_elements_);
        return meta_data_[index];
    }

    // Provide a label encoding compatible with GasFilterFunctor (meta in high 32 bits, id in low 32 bits).
    // This is only valid when MetaT fits in 32 bits.
    label_t get_label(size_t index) const {
        static_assert(std::is_integral_v<MetaT>, "MetaDataset::get_label requires integral MetaT");
        static_assert(sizeof(MetaT) <= 4, "MetaDataset::get_label requires MetaT <= 32 bits");
        assert(index < n_elements_);
...
inline auto load_qmeta<RangeGasFilterFunctor>(const std::string& path, size_t n_elements)
    ...
    if (!in.is_open()) {
        ...
        std::cerr << "Error: Failed to open range metadata file " << path << "hh" <<std::endl;
        std::exit(EXIT_FAILURE);
    }
...
public:
    ...
    void replace_filters(const std::string& meta_path) {
        filters_ = std::move(load_qmeta<FilterT>(meta_path, size()));
    }
...
// Composed queryset: loads one query vector file and two qmeta files, and returns a composed filter.
// Template parameter is ONLY the composed filter type.
template <typename ComposedFilterT>
requires gaslib::is_composed_filter_v<ComposedFilterT>
class FvecsComposedQueryset : public FvecsDataset {
public:
    using primary_filter_t = typename gaslib::composed_filter_traits<ComposedFilterT>::primary;
    using secondary_filter_t = typename gaslib::composed_filter_traits<ComposedFilterT>::secondary;

    FvecsComposedQueryset(size_t dim, size_t max_elements,
                          const std::string& query_path,
                          const std::string& qmeta1_path,
                          const std::string& qmeta2_path)
        : FvecsDataset(dim, max_elements, query_path),
          filters1_(load_qmeta<primary_filter_t>(qmeta1_path, size())),
          filters2_(load_qmeta<secondary_filter_t>(qmeta2_path, size())) {}

    ComposedFilterT get_filter(size_t index) const {
        assert(index < size());
        return ComposedFilterT(filters1_[index], filters2_[index]);
    }

private:
    std::vector<primary_filter_t> filters1_;
    std::vector<secondary_filter_t> filters2_;
};
