...
def generate_query_meta_range(output_file, n_query, selectivity, meta_min, meta_max, variety):
...
if __name__ == "__main__":
    ...
    if (len(sys.argv) > 0) and (len(sys.argv) < 7 or len(sys.argv) > 9):
        print(f"Usage: python {sys.argv[0]} <num_query> <output_path> <range/set> <selectivity*100> <range_min> <range_max> [variety]")
    ...
    if filter_type == 'range':
        generate_query_meta_range(output_file, n_query, selectivity, meta_min, meta_max, variety)
