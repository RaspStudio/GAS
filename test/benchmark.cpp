...
void build_all_indices(
    ...
    std::vector<size_t> build_times(indices.size(), 0);
...
auto load_ground_truth(
    ...
    std::cout << "----------------------------------------\n" << std::flush;
    std::cout << "Loading ground truth for " << n_queries << " queries, k=" << k << "\n" << std::flush;
    ...
    for (unsigned rep = 0; rep < repeat; ++rep) {
        ...
        for (size_t index_idx = 0; index_idx < indices.size(); ++index_idx) {
            ...
            for (size_t ef_idx = 0; ef_idx < efs.size(); ++ef_idx) {
                ...
                std::vector<size_t> hit_to_nquery(k + 1, 0);
...
void run_benchmark(
    ...
    consider([&]() { return std::make_unique<gaslib::GasIndex<DatasetT, FilterT, 0>>(dataset); });
    consider([&]() { return std::make_unique<gaslib::GasIndex<DatasetT, FilterT, 1>>(dataset); });
    consider([&]() { return std::make_unique<gaslib::GasIndex<DatasetT, FilterT, 2, true>>(dataset, 4, 12); });

    build_all_indices(dataset, indices, cache_dir, data_id, bmeta_id);

    auto gt = load_ground_truth<DatasetT, QuerysetT, FilterT>(queryset, k, cache_dir, data_id, bmeta_id, query_id);

    search_and_evaluate(queryset, indices, k, efs, gt, repeat, n_seg, batch_size, query_seq_mode);
...
void run_benchmark_composed(
    ...
    const std::string &cache_dir,
    const std::string &data_path,
    const std::string &bmeta1_path,
    const std::string &query_path,
    const std::string &qmeta1_path,
    const std::string &bmeta2_path,
    const std::string &qmeta2_path,
    unsigned int only_run_idx,
    unsigned int repeat,
    unsigned int n_seg,
    unsigned int batch_size,
    ...
    DatasetT dataset(dim, max_elements, data_path, bmeta1_path);
    gaslib::MetaSet metaset2 = gaslib::load_metaset(bmeta2_path, dataset.size());

    using ComposedFilterT = gaslib::ComposedFilterFunctor<FilterT1, FilterT2>;
    gaslib::FvecsComposedQueryset<ComposedFilterT> queryset(dim, max_queries, query_path, qmeta1_path, qmeta2_path);

    std::string data_norm   = std::filesystem::absolute(data_path).lexically_normal();
    std::string bmeta1_norm = std::filesystem::absolute(bmeta1_path).lexically_normal();
    std::string bmeta2_norm = std::filesystem::absolute(bmeta2_path).lexically_normal();
    std::string query_norm  = std::filesystem::absolute(query_path).lexically_normal();
    std::string qmeta1_norm = std::filesystem::absolute(qmeta1_path).lexically_normal();
    std::string qmeta2_norm = std::filesystem::absolute(qmeta2_path).lexically_normal();

    std::hash<std::string> hasher;
    std::string data_id  = std::to_string(hasher(data_norm + std::to_string(max_elements)));
    std::string bmeta_id = std::to_string(hasher(bmeta1_norm + "|" + bmeta2_norm + "|" + std::to_string(max_elements)));
    std::string query_id = std::to_string(hasher(query_norm + "|" + qmeta1_norm + "|" + qmeta2_norm));

    std::cout << "Data Path: " << data_norm << "\n"
              << "BMeta1 Path: " << bmeta1_norm << "\n"
              << "BMeta2 Path: " << bmeta2_norm << "\n"
              << "Dataset Elements: " << dataset.size() << "/" << max_elements << "\n"
              << "Query Path: " << query_norm << "\n"
              << "QMeta1 Path: " << qmeta1_norm << "\n"
              << "QMeta2 Path: " << qmeta2_norm << "\n"
              << "Queries: " << queryset.size() << "/" << max_queries << "\n"
              << "Data ID: " << data_id << "\n"
              << "BMeta ID: " << bmeta_id << "\n"
              << "Query ID: " << query_id << "\n";
    ...
    std::vector<std::unique_ptr<gaslib::IIndex<float, DatasetT, ComposedFilterT>>> indices;
    ...
    consider([&]() { return std::make_unique<gaslib::GasIndexExtend<DatasetT, ComposedFilterT, 0>>(dataset, &metaset2); });
    consider([&]() { return std::make_unique<gaslib::GasIndexExtend<DatasetT, ComposedFilterT, 1>>(dataset, &metaset2); });
    consider([&]() { return std::make_unique<gaslib::GasIndexExtend<DatasetT, ComposedFilterT, 2>>(dataset, &metaset2, 4, 12); });
    
    build_all_indices(dataset, indices, cache_dir, data_id, bmeta_id);

    auto gt = load_ground_truth<DatasetT, gaslib::FvecsComposedQueryset<ComposedFilterT>, ComposedFilterT>(queryset, k, cache_dir, data_id, bmeta_id, query_id);
...
int main(int argc, char **argv) {
    ...
    std::string bmeta_path  = argv[7];
    std::string query_path  = argv[8];
    std::string qmeta_path  = argv[9];

    auto is_uint_token = [](const char *s) -> bool {
        if (s == nullptr || *s == '\0') return false;
        char *end = nullptr;
        errno = 0;
        (void)std::strtoul(s, &end, 10);
        return errno == 0 && end != nullptr && *end == '\0';
    };

    bool has_composed_meta = false;
    std::string bmeta2_path;
    std::string qmeta2_path;

    int argi = 10;
    if (argc > argi && !is_uint_token(argv[argi])) {
        if (argc <= argi + 1) {
            std::cerr << "Error: composed mode requires two paths after qmeta_path: bmeta2_path qmeta2_path\n";
            return EXIT_FAILURE;
        }
        has_composed_meta = true;
        bmeta2_path = argv[argi];
        qmeta2_path = argv[argi + 1];
        argi += 2;
    }
    ...
    if (!has_composed_meta) {
        if (gaslib::qmeta_is_range(qmeta_path)) {
        ...
        } else if (gaslib::qmeta_is_tag(qmeta_path)) {
            ...
            std::cerr << "Error: unrecognized qmeta type in single meta mode: " << qmeta_path << "\n";
            return EXIT_FAILURE;
        }
    } else {
        bool q1_is_range = gaslib::qmeta_is_range(qmeta_path);
        bool q2_is_range = gaslib::qmeta_is_range(qmeta2_path);

        if (q1_is_range && q2_is_range) {
            std::cout << "Running Composed (Range + Range) Query Benchmark...\n";
            std::cout << "----------------------------------------\n" << std::flush;
            run_benchmark_composed<
                gaslib::FvecsDatasetWithMeta,
                gaslib::RangeGasFilterFunctor,
                gaslib::RangeGasFilterFunctor
            >(dim, max_elements, max_queries, k, efs, cache_dir,
                ...
                bmeta2_path, qmeta2_path,
                only_run_idx, repeat, n_seg, batch_size, query_seq_mode);
        } else if (q1_is_range && !q2_is_range) {
            std::cout << "Running Composed (Range + Tag) Query Benchmark...\n";
            std::cout << "----------------------------------------\n" << std::flush;
            run_benchmark_composed<
                gaslib::FvecsDatasetWithMeta,
                gaslib::RangeGasFilterFunctor,
            ...
            std::cout << "Running Composed (Tag + Range) Query Benchmark...\n";
            std::cout << "----------------------------------------\n" << std::flush;
            run_benchmark_composed<
                gaslib::FvecsDatasetWithMeta,
                gaslib::TagGasFilterFunctor,
            ...
            std::cout << "Running Composed (Tag + Tag) Query Benchmark...\n";
            std::cout << "----------------------------------------\n" << std::flush;
            run_benchmark_composed<
                gaslib::FvecsDatasetWithMeta,
                gaslib::TagGasFilterFunctor,
