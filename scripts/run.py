...
# -*- coding: utf-8 -*-
"""
Batch runner for search experiments with NUMA-aware binding.

- Automatically detects NUMA nodes and their CPU lists.
- Assigns each launched benchmark subprocess to a specific NUMA node (compute + memory binding).
- Optionally limits CPUs per task and sets OMP_NUM_THREADS accordingly.
- Falls back to taskset if numactl is unavailable.
- Preserves your original logging directory structure and CLI layout.
...
# -----------------------------------------------------------------------------
# NUMA physical-core selection helpers
# -----------------------------------------------------------------------------

_CPU_SIBLING_CACHE: Dict[int, List[int]] = {}

def _get_thread_siblings(cpu: int) -> List[int]:
    """Return list of sibling CPUs (SMT threads) for a given cpu id."""
    if cpu in _CPU_SIBLING_CACHE:
        return _CPU_SIBLING_CACHE[cpu]
    p = Path(f"/sys/devices/system/cpu/cpu{cpu}/topology/thread_siblings_list")
    if not p.exists():
        _CPU_SIBLING_CACHE[cpu] = [cpu]
        return [cpu]
    s = p.read_text().strip()
    sibs = _expand_cpulist(s)
    if not sibs:
        sibs = [cpu]
    _CPU_SIBLING_CACHE[cpu] = sibs
    return sibs

def _pick_physical_cores_from_cpus(cpus: List[int]) -> List[int]:
    """Given CPUs for a NUMA node (may include SMT), pick one CPU per physical core."""
    if not cpus:
        return []
    cpu_set = set(cpus)
    reps: Dict[int, int] = {}  # rep -> rep
    seen_cores: set = set()

    # define a 'core key' as the smallest cpu id among siblings within this cpu_set
    for c in sorted(cpus):
        sibs = [x for x in _get_thread_siblings(c) if x in cpu_set]
        if not sibs:
            sibs = [c]
        core_key = min(sibs)
...
def run_experiment_batch(
    ...
    batch_size: int = 1,
    query_seq_mode: str = "normal",
    log_root_dir: str = "logs",
    group_name: Optional[str] = None,
    max_threads: int = 1,
    # NUMA binding options
    use_physical_cores: bool = True,    
    bind_numa: bool = True,
    ...
    if allowed_numa_nodes is not None:
        ...
        if not numa_nodes:
            raise RuntimeError("allowed_numa_nodes are null after filtering")

    def _slice_cpulist_for_task(cpus_array: List[int]) -> str:
        # Prefer physical cores (avoid SMT siblings) unless user disables it.
        base = cpus_array
        if use_physical_cores:
            base = _pick_physical_cores_from_cpus(cpus_array)

        if cpus_per_task is None or cpus_per_task >= len(base):
            return _compress_cpulist(base)
        return _compress_cpulist(base[:cpus_per_task])
    ...
    def run_single(job: dict) -> str:
        ...
        main_bmeta_path = job["bmeta_path"]
        main_qmeta_path = job["qmeta_path"]
        
        if type(main_bmeta_path) == list and type(main_bmeta_path) == list:
            main_bmeta_path = main_bmeta_path[0]
            main_qmeta_path = main_qmeta_path[0]
        ...
        cmd = [
            ...
            str(job["max_queries"]),
            str(job["k"]),
            os.path.expanduser(job["cache_dir"]),
            os.path.expanduser(ds.data_path),
            os.path.expanduser(main_bmeta_path),
            os.path.expanduser(ds.query_path),
            os.path.expanduser(main_qmeta_path),
        ]

        if type(job["bmeta_path"]) == list and type(job["qmeta_path"]) == list:
            cmd.append(str(job["bmeta_path"][1]))
            cmd.append(str(job["qmeta_path"][1]))
        ...
        _append_positional_tail(
            ...
            query_seq_mode=job["query_seq_mode"],
            ef_list=ef_list
        )
        ...
        b_tag = ""
        if type(job["bmeta_path"]) == list:
            for bp in job["bmeta_path"]:
                b_tag = b_tag + "-" + _slug(_short_path(bp))
        else:
            b_tag = _slug(_short_path(job["bmeta_path"]))

        q_tag = ""
        if type(job["qmeta_path"]) == list:
            for qp in job["qmeta_path"]:
                q_tag = q_tag + "-" + _slug(_short_path(qp))
        else:
        ...
        if bind_numa and set_omp_threads and job["numa_node"] is not None:
            if "OMP_NUM_THREADS" not in env and job["cpu_list_str"]:
                omp_threads = 144 # len(_expand_cpulist(job["cpu_list_str"]))
                if omp_threads > 0:
